<template>
    <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
        ...
        <div class="form-group has-feedback" :class="{ 'has-error': form.errors.has('email') }">
            <input type="email" class="form-control" placeholder="Enter Email" name="email" v-model="form.email"
                   autofocus/>
        ...
        <div class="row">
            ...
            <div class="col-xs-8">
                <button type="submit" class="btn btn-primary btn-block btn-flat" :disabled="form.errors.any()"><i
                        v-if="form.submitting" class="fa fa-refresh fa-spin"></i> Send Password Reset Link
                </button>
            </div><!-- /.col -->
            <div class="col-xs-2">
            </div><!-- /.col -->
        ...
        methods: {
            submit() {
                this.form.post('/password/email')
                    ...
                    .catch(error => {
                        console.log('password sent' + ':' + error)
                    })
            },
            clearErrors(name) {
